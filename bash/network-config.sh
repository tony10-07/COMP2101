...
#
My_host=$(hostname)
Lan_Adrs=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
Lan_host=$(getent hosts $Lan_Adrs| awk '{print $2}') 
...
Rotr_Add=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
Rotr_Host=$(getent hosts $Rotr_Add | awk '{print $2}')
...
cat <<EOF
Hostname        : $My_host
LAN Address     : $Lan_Adrs
LAN Hostname    : $Lan_host
External IP     : $Ext_IP
External Name   : $Ext_Name
Router Address	: $Rotr_Add
Router Hostname	: $Rotr_Host
EOF
